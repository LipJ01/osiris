...
async function fetchAdsbLol(lat: number, lon: number, dist: number): Promise<any[]> {
  try {
    const res = await stealthFetch(`https://api.adsb.lol/v2/lat/${lat}/lon/${lon}/dist/${dist}`, {
...
async function fetchAdsbFi(lat: number, lon: number): Promise<any[]> {
  try {
    const res = await stealthFetch(`https://opendata.adsb.fi/api/v2/lat/${lat}/lon/${lon}/dist/${FALLBACK_DIST}`, {
