...
/**
 * OSIRIS — Network Interference (OONI)
 *
 * Pulls measurement aggregates from OONI Probe network — the open observatory
 * for global internet censorship + interference. Each row is `(country, day,
 * counts)`; we aggregate across the rolling 7-day window per country and
 * surface countries with elevated anomaly + confirmed-block rates.
 *
 * Anomaly = automated heuristics suggesting interference; Confirmed = OONI
 * verified blocking matching a known fingerprint (DNS hijack, SNI block,
 * etc.). Both signals matter for the OSINT picture.
 *
 * Server-side cached 6h. OONI runs ~12M measurements per week worldwide.
 */

const TTL_MS = 6 * 60 * 60 * 1000;
const CACHE_KEY = 'network-interference';
const WINDOW_DAYS = 7;
const MIN_MEASUREMENTS = 100;     // statistical floor for inclusion

// ISO2 → centroid for the ~110 countries that show up regularly in OONI data
...
interface CountryAggregate {
  ...
  ok_count: number;
  anomaly_rate: number;       // (anomaly + confirmed) / measurements
...
function severity(rate: number, confirmed: number): CountryAggregate['severity'] {
  if (rate >= 0.20 || confirmed >= 100) return 'critical';
  if (rate >= 0.07 || confirmed >= 10) return 'elevated';
  return 'normal';
}
...
export async function GET() {
  ...
  try {
    ...
    const u = new URL('https://api.ooni.org/api/v1/aggregation');
    u.searchParams.set('since', sinceStr);
    u.searchParams.set('until', untilStr);
    u.searchParams.set('axis_x', 'measurement_start_day');
    u.searchParams.set('axis_y', 'probe_cc');
    u.searchParams.set('test_name', 'web_connectivity');
    ...
    // Aggregate per country across the rolling window
    const agg = new Map<string, any>();
    for (const r of rows) {
      const iso = (r.probe_cc || '').toUpperCase();
      if (!iso) continue;
      if (!agg.has(iso)) {
        agg.set(iso, { iso, measurement: 0, anomaly: 0, confirmed: 0, failure: 0, ok: 0 });
      }
      const a = agg.get(iso);
      a.measurement += r.measurement_count || 0;
      a.anomaly += r.anomaly_count || 0;
      a.confirmed += r.confirmed_count || 0;
      a.failure += r.failure_count || 0;
      a.ok += r.ok_count || 0;
    }
    ...
    for (const a of agg.values()) {
      totalMeas += a.measurement;
      if (a.measurement < MIN_MEASUREMENTS) continue;
      const coord = C[a.iso];
      if (!coord) continue;
      const rate = (a.anomaly + a.confirmed) / a.measurement;
      countries.push({
        iso: a.iso,
        name: coord.name,
        lat: coord.lat,
        lng: coord.lng,
        measurement_count: a.measurement,
        anomaly_count: a.anomaly,
        confirmed_count: a.confirmed,
        failure_count: a.failure,
        ok_count: a.ok,
        anomaly_rate: rate,
        severity: severity(rate, a.confirmed),
      });
    }
    countries.sort((a, b) => b.anomaly_rate - a.anomaly_rate);
    // Surface only elevated + critical to keep the map readable
    const visible = countries.filter(c => c.severity !== 'normal');
    ...
    return NextResponse.json(
      { error: 'Failed to fetch OONI data', detail: e instanceof Error ? e.message : String(e) },
      { status: 502 },
    );
  }
}
