...
import {
  ...
  CloudLightning, Radiation, Tv, Anchor, Ship, Fish, Fuel, Pickaxe, Trees, Landmark, Cable, Rocket, Wind, Biohazard, Megaphone, Network, Bug, Tent, Container, Train, Package, Server, Cpu, Mountain, WifiOff, Snowflake, Pill, Ban,
...
function LayerPanel({ data, activeLayers, setActiveLayers }: LayerPanelProps) {
  ...
  const allLayersActive = ALL_LAYERS.every(l => activeLayers[l.key]);
  const toggleAll = () => {
    setActiveLayers((prev: any) => {
      const next = { ...prev };
      ALL_LAYERS.forEach(l => { next[l.key] = !allLayersActive; });
      return next;
    });
  };
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            title={allLayersActive ? 'Disable all layers' : 'Enable all layers'}
          >
            {allLayersActive ? (
              <ToggleRight className="w-4 h-4 text-[var(--gold-primary)]" />
            ) : (
